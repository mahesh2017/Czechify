...
## Delivery 10 — Fixes from the Sep 23 review

- **Purchases the app never sent in (`20261002100000_purchase_ownership.sql`, `play_discovery.ts`):** Play refunds a subscription nobody acknowledges within three days, and only the app could register one. A payment that completed later, a failed check or a closed screen left the purchase to expire. Now a Play notification for an unknown subscription token keeps the token (encrypted) in `play_purchase_discoveries`; the worker asks Play which account started the purchase, by the obfuscated ID only this server hands out, registers it for that account, verifies it and acknowledges it. Tokens are dropped once resolved and rows deleted after 30 days.
- **Google Play decides ownership:** the first account to send a token in used to own it even when Play named another account, so the real buyer was sent to support. A purchase not yet confirmed for anyone now moves to the account Play names, open cases about it close (`decided_by = google_play`), and the request that ran the check is told only `account_binding_mismatch`. A buyer restoring a token another account holds queues a fresh check. Once confirmed, a purchase never moves without support.
- **Invitation deadlock:** delivery 9 ordered the locks inside claim processing, but lesson receipts and review decisions still locked the inviter first before calling it, so two people who invited each other could deadlock through the receipt path. `private.lock_referral_accounts` now takes both accounts in ID order for all three. The new `mutual_receipts` case in `tool/test_referral_concurrency.py` fails with `deadlock detected` on the previous receipt function and passes now; delivery 9's test called claim processing directly and missed this.
...
- **Retention:** a deleted account's purchase whose last known state was active, in grace, cancelled, on hold or paused was never deleted, because nothing re-checks it once the owner is gone. It now goes 120 days after its last paid-through date. Tokens that never became a purchase go after 30 days unless still being checked or under a support case.
