...
- A notification for a token no account has registered is kept as `unmatched`. A subscription token is also queued for discovery (PR 10): the worker asks Play which account started the purchase and registers, verifies and acknowledges it for that account, so a purchase the app never sent in is not refunded. This needs `MONETIZATION_TOKEN_KEY` available to `play-billing-notifications`; without it, only the owner's own verify call provisions. A notification that arrives while a refresh is running queues one more behind it, because the running one may have read Play before the change.
- `POST purchases/verify` allows 30 checks per account an hour (`allow_purchase_verification`); more answer `429 rate_limited`. Each check may call the Play Developer API, whose quota the whole app shares.
- Each worker stage (billing, discoveries, referrals, AI retention, privacy retention, operations report) runs even if an earlier one failed. A failed stage logs `monetization_worker_stage_failed`, is sent as a `worker_<stage>_failed` alert and makes the run answer 503.
