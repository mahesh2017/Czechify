...
4. In a transaction, lock the `monetization_accounts` rows **before** claim/milestone rows. Since the friend's trial (delivery 9) one claim writes to both accounts, so every receipt, review and allocation locks both accounts of the claim in ascending account ID (`private.lock_referral_accounts`, delivery 10). Code that writes one account locks only that one.
...
3. Fetch Play outside a long-running SQL transaction. Validate product/package/state and authoritative account binding. If token is already owned by another account, return a generic conflict without reassigning it on the caller's say-so. Never bind a new token solely because a caller knows it. Play's report is the exception (delivery 10): until a purchase has been confirmed for anyone, it belongs to the account whose obfuscated ID Play names, and a conflicting sender queues a fresh Play check instead of only a support case. The request that ran the check learns nothing about a purchase that moved away.
...
After authentication, transactionally insert the unique inbox record and job, then acknowledge transport with a success response. A subscription token no account has registered is also queued (encrypted) for discovery: the worker asks Play for its obfuscated account ID and registers, verifies and acknowledges the purchase for that account, so nobody's purchase depends on the app sending it in (delivery 10). If persistence fails, return an error for redelivery. An already durably stored duplicate gets success. Processing failure after durable intake uses the internal retry/dead-letter queue. Never return success before durable storage or wait for all Play/API work before accepting the message.

## 5. Retention, recovery and operations

Proposed operational retention: raw normalized referral coverage 90 days after terminal qualification; transient Integrity tokens zero persistence; nonce rows 24 hours after expiry; AI replay content 24 hours; content-free AI request tombstones seven days; sanitized operational logs 30 days. Keep minimal grant/milestone audit records while they explain an active grant. Delete linked learner detail on account deletion. Billing retention is decided (see [PRIVACY_AND_DATA_SAFETY.md](PRIVACY_AND_DATA_SAFETY.md#decisions-22-sep-2026)): an ownerless purchase stays only while Google Play could restore it, then 30 days. Google's payout reports, not these rows, are the bookkeeping documents. `cleanup_privacy_records` enforces this, the 90-day referral and legacy-snapshot periods, and 30 days for operational rows.
