...
| `POST /referrals/claim` | `{campaign_id,code,attribution_source:"app_link"|"install_referrer"|"manual"}` | `201 {claim_id,status,required_units:[1,2]}`. Same referrer retry returns existing claim. Different referrer returns `409 referral_already_claimed`. |
| `POST /referrals/challenges` | `{claim_id,receipt_digest}` | `{nonce,expires_at}` for the authenticated referee; rate-limited. |
| `POST /referrals/receipts` | Envelope below | `202 {receipt_id,status:"queued"}` or replay of final accepted status. Claim ownership required. Max body 64 KiB. |
| `GET /referrals/status` | Cursor for own referral summaries | Permanent units earned, next reward unit/null, own invitee claim and privacy-safe referred-friend progress; cursor pagination 20, max 100. |
