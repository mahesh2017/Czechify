...
- Granted in `process_referral_claim` once both free units are qualified, under the same evidence, review and pause rules (`private.referral_trial_due`). A held or rejected invitation, or a friend who is not linked, gets no trial. Since PR 12 the inviter's side does not matter: their 15-unit cap, a missing link, or a deleted account never stops the friend's trial, and a friend whose inviter was deleted can still send lesson results.
- The length is the campaign's `referee_trial_days` (14), read by both processing and `get_referral_status`.
...
- **No mutual invitations (PR 12).** `claim_referral` refuses a code whose owner the caller invited: two new accounts would each earn units and a trial from one another. Deleting and recreating an account, or opening many Google accounts, can still earn a new trial each time; that was a deliberate choice not to keep any record of deleted accounts.
- **Lock order.** A claim now writes to both accounts, so two invitations can touch the same pair from opposite sides (A invited B, B invited A). Both transactions lock the **lower account ID first**, whichever side of their claim it is on. `tool/test_referral_concurrency.py` covers this: with the previous beneficiary-first order the same test fails with `deadlock detected`. Lesson receipts and review decisions take the same order (PR 10). Mutual claims are refused now, but the contention tests write the reverse claim directly, because ordered locking is what keeps any two claims over one pair safe.
- **Device-check choice (PR 12).** The app asks for the Play Integrity choice when the friend enters a code, and lesson results wait on the device until they answer; one result sent unchecked would put the whole invitation into manual review. The consent wording is `referral-integrity-v2`.
